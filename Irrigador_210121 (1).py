...
GPIO.add_event_detect(solo_pin, GPIO.BOTH, callback=atualizar_led_umidade, bouncetime=200)
...
while True:
    ...
    if hora_agora < 8:
        ...
        if GPIO.input(solo_pin) == 1:
            ...
            now = datetime.datetime.now()
            tempo_espera = 21600
            tempo_bomba_desligada = (tempo_espera/3600)
            ...
            #GPIO.output(led_solo_pin, GPIO.HIGH)
            atualizar_led_umidade()
            tempo_desligada_umido = 7200
            print(f"Solo úmido... Bomba desligada por {tempo_desligada_umido / 60} minutos. Agora são: {now.strftime(f'%H:%M:%S')}")
            for restante in range(tempo_desligada_umido, 0, -1):
                horas = restante // 3600
                minutos = (restante % 3600) // 60
                segundos = restante % 60
                print(f"\r Tempo restante até religar: {horas:02d}:{minutos:02d}:{segundos:02d}", end="", flush=True)
                time.sleep(1)
            #time.sleep(tempo_desligada_umido)  # Tempo de espera para recomeçar a análise do solo = 2 horas, 6 horas
        ...
        print(f"Próxima tentativa de ligar a bomba em {tempo_tentativa_bomba / 60} minutos!")
        for restante in range(tempo_tentativa_bomba, 0, -1):
                horas = restante // 3600
                minutos = (restante % 3600) // 60
                segundos = restante % 60
                print(f"\r Tempo restante até religar: {horas:02d}:{minutos:02d}:{segundos:02d}", end="", flush=True)
                time.sleep(1)
        #time.sleep(tempo_tentativa_bomba)  # Tempo de espera para próximo aviso!
